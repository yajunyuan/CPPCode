...
__global__ void SearchDefect1(int* da, int row_a, int col_a, int* result)
{
    ...
    if (row % 128 == 0 && (row + 128) <= row_a && col % 128 == 0 && (col + 128) <= col_a)
    //if (row % 100 == 0 && (row ) <= row_a && col % 100 == 0 && (col) <= col_a)
    {
        int SingleImage_count = 0;
        for (int i = row; i < row + 128; i++) {
...
extern "C" void SearchDefectFunc(int* da, int row_a, int col_a, double upper, double lower, int* result, cudaStream_t streams[])
{
    ...
    SearchDefect << <dgs, dbs , 0, streams[1]>> > (da, row_a, col_a,result);
}

extern "C" void SearchDefectFunc1(int* da, int row_a, int col_a, int* result, cudaStream_t streams[])
{
    SearchDefect1 << <dgs, dbs, 0, streams[1] >> > (da, row_a, col_a, result);
}

__global__ void reset(int* result)
{
    int threadId = blockIdx.x * blockDim.x + threadIdx.x;
    if (threadId == 0) {
        atomicSub((int*)&resultIndex, result[0]);
    }

}
extern "C" void Reset(int* result, cudaStream_t streams[])
{
    reset << <1, 1, 0, streams[1] >> > (result);
}
