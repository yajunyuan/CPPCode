...
PPOCR::PPOCR(std::string det_model_dir, std::string rec_model_dir) {
  if (true) {
    this->detector_ = new DBDetector(
        det_model_dir, false, 0, 4000,
        10, false, "max",
        960, 0.3, 0.6,
        1.5, "slow", false,
        false, "fp32");
  ...
  if (true) {
    this->recognizer_ = new CRNNRecognizer(
        rec_model_dir, false, 0, 4000,
        10, false, "./ppocr_keys_v1.txt",
        false, "fp32", 6,
        48, 320);
  }
};
